...
#Scripts for artificially creating conversations out of VoxPopuli
...
LIST_FILE_TR=$LISTS_DIR/train.str
LIST_FILE_VAL=$LISTS_DIR/validation.str
CONV_VAL_LIST=$LISTS_DIR/conv_list_val
CONV_TR_LIST=$LISTS_DIR/conv_list_tr
# STATS_DIR=$WORKDIR/stats
STATS_DIR=$WORKDIR/stats_DIHARD3cts
...
# if [ ! -f $LIST_FILE_TR ]; then
#     echo "Processing segments"
#     # process segments train
#     grep -F -f <(awk '{print $1}' $UTT2SPK_TR) <(awk '{print $2" "$3" "$4}' $SEG_FILE) | awk -F"_" '{print $1"_"$2" "$0}' > $LIST_FILE_TR
#     # process segments validation
#     grep -F -f <(awk '{print $1}' $UTT2SPK_VAL) <(awk '{print $2" "$3" "$4}' $SEG_FILE) | awk -F"_" '{print $1"_"$2" "$0}' > $LIST_FILE_VAL
...
#         --seg-list-file $LIST_FILE_VAL --out-conv-dir $CONV_VAL_DIR \
...
#     mkdir -p $CONV_TR_DIR
#     python3 $DIR/conv_generator.py --stats-dir $STATS_DIR \
#         --seg-list-file $LIST_FILE_TR --out-conv-dir $CONV_TR_DIR \
#         --determine-spks $DETSPKS --num-spks $NUMSPKS \
...
if [[ ! -d $WAVS_TR_DIR ]]; then
    ...
    for f in `find $LISTS_DIR/convtrsublists/conv.lst_*`; do
        ...
        echo "python $DIR/conv2wav.py --conversations-list-filename $f --input-wav-scp $WAV_SCP_TR --in-conv-dir $CONV_TR_DIR --out-wav-dir $WAVS_TR_DIR $use_rirs $use_noises --noises-snrs $SNRS --sampling-frequency $SAMPLINGFREQ" >> $SET_DIR/conv2wav.task
    done
    ...
    echo "python $DIR/conv2wav.py --conversations-list-filename $CONV_VAL_LIST --input-wav-scp $WAV_SCP_VAL --in-conv-dir $CONV_VAL_DIR --out-wav-dir $WAVS_VAL_DIR $use_rirs $use_noises --noises-snrs $SNRS --sampling-frequency $SAMPLINGFREQ" >> $SET_DIR/conv2wav.task
